...
class ProjectManager:
    ...
    def get_project_directory(self, project_name: str) -> str:
        ...
        all_dirs = [d for d in os.listdir(self.base_dir) 
                   if os.path.isdir(os.path.join(self.base_dir, d)) and d.startswith(clean_name)]
        
        if not all_dirs:
            return None
        
        # Sort by creation time (newest first)
        all_dirs.sort(key=lambda x: os.path.getctime(os.path.join(self.base_dir, x)), reverse=True)
