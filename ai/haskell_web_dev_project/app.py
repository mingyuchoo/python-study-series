...
# Initialize OpenAI API configuration from .env file
...
# Format the API key if it exists in the environment
if os.environ.get("OPENAI_API_KEY"):
    os.environ["OPENAI_API_KEY"] = format_openai_key(os.environ.get("OPENAI_API_KEY"))
...
st.markdown("""This application uses CrewAI to implement a multi-agent collaboration system 
...
with st.sidebar:
    ...
    step_status = {
        "Requirements Analysis": "✅" if "analysis" in st.session_state.results else "⏳",
        "Design": "✅" if "design" in st.session_state.results else "⏳",
        "Implementation": "✅" if "implementation" in st.session_state.results else "⏳",
...
if st.session_state.current_step == "input":
    ...
    project_dir = st.session_state.project_manager.create_project_directory(st.session_state.project_name)
    ...
    try:
        ...
        # Set up LLM configuration with the user-provided API settings
        # Note: Individual agent files now use the get_openai_config() function
        # This reference is kept for documentation purposes
        ...
        analyzer_tasks = create_analyzer_tasks(analyzer_agent, st.session_state.project_name, st.session_state.project_description, project_dir)
        ...
        try:
            ...
            designer_tasks = create_designer_tasks(designer_agent, st.session_state.project_name, analysis_result, project_dir)
            design_crew = Crew(
            ...
            developer_tasks = create_developer_tasks(developer_agent, st.session_state.project_name, design_result, project_dir)
            ...
            tester_tasks = create_tester_tasks(tester_agent, st.session_state.project_name, implementation_result, project_dir)
            testing_crew = Crew(
                agents=[tester_agent],
                tasks=tester_tasks,
                verbose=True,
            ...
            deployer_tasks = create_deployer_tasks(deployer_agent, st.session_state.project_name, testing_result, project_dir)
            ...
            if "AuthenticationError" in error_message or "invalid_api_key" in error_message:
                st.session_state.error_message = "Authentication error: Please check your OpenAI API key."
        ...
        st.session_state.error_message = f"Error setting up the development process: {str(e)}"
        st.session_state.current_step = "input"
