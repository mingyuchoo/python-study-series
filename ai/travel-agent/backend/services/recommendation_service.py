...
from backend.models.response_models import TravelPackage, SessionResponse
...
class RecommendationService:
    ...
    def create_session(self, user_ip: str = None, user_agent: str = None) -> SessionResponse:
        """새 세션 생성"""
    ...
    def save_answer(self, session_id: str, question_id: int, 
                   answer_text: str = None, answer_value: str = None) -> bool:
        """답변 저장"""
        # 기존 답변 확인
        check_query = """
        ...
        if existing:
            ...
            WHERE session_id = ? AND question_id = ?
            """
            db_manager.execute_insert(query, (answer_text, answer_value, session_id, question_id))
        else:
            # 새 답변 삽입
            query = """
            ...
            VALUES (?, ?, ?, ?)
            """
            db_manager.execute_insert(query, (session_id, question_id, answer_text, answer_value))
    ...
    def get_user_answers(self, session_id: str) -> Dict[int, str]:
        ...
        for answer in answers_data:
            answers[answer['question_id']] = answer['answer_value'] or answer['answer_text']
    ...
    def get_recommendations(self, session_id: str, limit: int = 5) -> List[TravelPackage]:
        """개인화된 추천 생성"""
        # 사용자 답변 조회
        ...
        # Vector Store를 통한 추천
        recommendations = self.vector_store.get_personalized_recommendations(user_answers)

        # 유사도 기준(>= threshold)으로 필터링 후 limit 적용
        filtered_recs = [rec for rec in recommendations if rec.get('similarity_score') is not None and rec['similarity_score'] >= self._min_similarity_threshold]
