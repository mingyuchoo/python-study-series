...
from cli_main import graph, ResearchState
...
if submitted and topic.strip():
            ...
            if node in agent_names:
                with st.expander(f"{agent_names[node]} 결과 보기", expanded=True):
                    if node == "research_topic":
                        ...
                        st.text_area("초안 전체 내용 (수정 가능)", value=value.get('draft', ''), key=f"draft_edit_{node}", height=200)
                        ...
                        st.text_area("피드백 전체 내용 (수정 가능)", value=value.get('feedback', ''), key=f"feedback_edit_{node}", height=200)
                        ...
                        st.text_area("최종 보고서 전체 내용 (수정 가능)", value=value.get('final_report', ''), key=f"final_report_edit_{node}", height=300)
    ...
    st.session_state["history"].append({"topic": topic, "report": latest_state.get("final_report", "")})
